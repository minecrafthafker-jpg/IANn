// race.js - external brand logos, compact UI adjustments (uses existing firestore integration)
(function(){
  ...
  function fitCanvas(){
    ...
    canvas.width = w; canvas.height = Math.floor(w * (800/480));
  }
  ...
  // brands + external logo URLs (Wikimedia or placeholder). If a URL fails, images show placeholder.
  const brands = [
  ...
  // prepare image elements for brands
  const brandImgs = brands.map(b=>{ const img=new Image(); img.crossOrigin='anonymous'; img.src=b.url; img.onerror=()=>{ img.src = `https://via.placeholder.com/160x120.png?text=${encodeURIComponent(b.name)}`; }; return img; });
  ...
  function update(){ if(!running) return; frame++; spawnTimer++; if(spawnTimer>=spawnInterval){ spawnTimer=0; spawnObstacle(); if(Math.random()<0.06) spawnPowerup(); if(frame%600===0 && spawnInterval>30) spawnInterval-=5; if(frame%400===0) speed+=0.3; } for(const o of obstacles) o.y+=speed; obstacles=obstacles.filter(o=>o.y<canvas.height+200); if(frame%30===0) score++; for(const o of obstacles){ if(!o.scored && o.y>car.y+car.h){ o.scored=true; player.points+=2; saveLocal(); saveRemote(); updateUI(); } if(rectIntersect(o.x,o.y,o.w,o.h, car.x,car.y,car.w,car.h)){ gameOver(); return; } } }

  function rectIntersect(x1,y1,w1,h1,x2,y2,w2,h2){ return !(x2 > x1 + w1 || x2 + w2 < x1 || y2 > y1 + h1 || y2 + h2 < y1); }

  function draw(){ ctx.fillStyle='#071022'; ctx.fillRect(0,0,canvas.width,canvas.height); for(let i=0;i<laneCount;i++){ const lx=i*laneWidth; ctx.fillStyle = i%2===0? '#07182a':'#061425'; ctx.fillRect(lx,0,laneWidth,canvas.height); if(i<laneCount-1){ ctx.strokeStyle='rgba(255,255,255,0.06)'; ctx.setLineDash([16,12]); ctx.beginPath(); ctx.moveTo(lx+laneWidth-2,0); ctx.lineTo(lx+laneWidth-2,canvas.height); ctx.stroke(); ctx.setLineDash([]); } } for(const o of obstacles){ ctx.fillStyle='#e85a5a'; ctx.fillRect(o.x,o.y,o.w,o.h); } const sel = car.selected || 1; const img = brandImgs[sel-1]; if(img && img.complete){ ctx.drawImage(img, car.x, car.y, car.w, car.h); } else { ctx.fillStyle='#ffd966'; ctx.fillRect(car.x,car.y,car.w,car.h); } }

  function loop(){ update(); draw(); scoreEl.textContent=score; rafId=requestAnimationFrame(loop); }

  function gameOver(){ running=false; cancelAnimationFrame(rafId); finalScoreEl.textContent=score; overlay.classList.remove('hidden'); const earned = Math.floor(score/5); player.points += earned; saveLocal(); saveRemote(); updateUI(); }
  ...
  btnRestart.addEventListener('click', restart); btnLeaderboard.addEventListener('click',()=>{ window.location.href='leaderboard.html'; });
