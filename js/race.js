// race.js - local logos, smaller canvas, WebAudio, Top-10 sidebar
// Updated: require initial name input before game starts
(function(){
  // audio helper
  ...
  const initialNameInput = document.getElementById('initialName');
  ...
  let obstacles=[]; let powerups=[]; let spawnTimer=0, spawnInterval=70, speed=3, frame=0; let score=0, running=false, rafId=null;
  let shake = 0;

  let player={ id:null, name:null, points:0, owned:[1], selected:1 };

  // local logo files (must exist in assets/cars/)
  const brandImgs = [];
  for(let i=1;i<=10;i++){ const img = new Image(); img.src = `assets/cars/car${i}.svg`; img.onerror = ()=>{ /* fallback handled */ }; brandImgs.push(img); }
  ...
  function ensurePlayer(){ let id = localStorage.getItem('playerId'); if(!id){ id = 'p_'+Math.random().toString(36).slice(2,10); localStorage.setItem('playerId', id);} player.id=id; const stored = localStorage.getItem('playerData'); if(stored){ try{ const p=JSON.parse(stored); if(p.id===player.id) player = p;}catch(e){} } if(!player.owned||player.owned.length===0) player.owned=[1]; if(!player.selected) player.selected = player.owned[0]; // name may be missing -> we show name overlay
    if(window.db){ window.db.collection('users').doc(player.id).get().then(doc=>{ if(doc.exists){ const d=doc.data(); player.points=d.points||player.points; player.owned=d.owned||player.owned; player.selected=d.selected||player.selected; player.name = d.name || player.name; saveLocal(); } else { // create with name if present
          const data = { points: player.points, owned: player.owned, selected: player.selected, createdAt: new Date().toISOString() };
          if(player.name) data.name = player.name;
          window.db.collection('users').doc(player.id).set(data).catch(e=>console.warn(e)); }
      }).catch(e=>console.warn(e)); }
  }

  function saveLocal(){ localStorage.setItem('playerData', JSON.stringify(player)); updateUI(); }
  async function saveRemote(){ if(!window.db) return; try{ const doc = { points:player.points, owned:player.owned, selected:player.selected, updatedAt:new Date().toISOString() }; if(player.name) doc.name = player.name; await window.db.collection('users').doc(player.id).set(doc, { merge:true }); }catch(e){console.warn(e);} }
  ...
  function update(){ if(!running) return; frame++; spawnTimer++; if(spawnTimer>=spawnInterval){ spawnTimer=0; spawnObstacle(); spawnPowerup(); if(frame%600===0 && spawnInterval>30) spawnInterval-=5; if(frame%400===0) speed+=0.25; } for(const o of obstacles) o.y+=speed; for(const p of powerups) p.y+=speed; obstacles=obstacles.filter(o=>o.y<canvas.height+200); powerups=powerups.filter(p=>p.y<canvas.height+100); if(frame%30===0){ score += car.double>0 ? 2 : 1; } for(const o of obstacles){ if(!o.scored && o.y>car.y+car.h){ o.scored=true; player.points += car.double>0 ? 4 : 2; saveLocal(); saveRemote(); updateUI(); } if(rectIntersect(o.x,o.y,o.w,o.h, car.x,car.y,car.w,car.h)){ if(car.shield>0){ car.shield=0; obstacles = obstacles.filter(x=>x!==o); continue; } playCollision(); shake = 12; gameOver(); return; } } for(const p of powerups){ if(rectIntersect(p.x,p.y,p.size,p.size, car.x,car.y,car.w,car.h)){ applyPowerup(p.type); powerups = powerups.filter(x=>x!==p); } } if(car.double>0) car.double--; if(car.shield>0) car.shield--; if(shake>0) shake = Math.max(0, shake - 1); }
  ...
  function draw(){ const ox = (Math.random()-0.5) * shake; const oy = (Math.random()-0.5) * shake; ctx.setTransform(1,0,0,1,ox,oy); ctx.fillStyle='#071022'; ctx.fillRect(0,0,canvas.width,canvas.height); for(let i=0;i<laneCount;i++){ const lx=i*laneWidth; ctx.fillStyle = i%2===0? '#07182a':'#061425'; ctx.fillRect(lx,0,laneWidth,canvas.height); if(i<laneCount-1){ ctx.strokeStyle='rgba(255,255,255,0.06)'; ctx.setLineDash([12,10]); ctx.beginPath(); ctx.moveTo(lx+laneWidth-2,0); ctx.lineTo(lx+laneWidth-2,canvas.height); ctx.stroke(); ctx.setLineDash([]); } } for(const o of obstacles){ ctx.fillStyle='#e85a5a'; ctx.fillRect(o.x,o.y,o.w,o.h); } for(const p of powerups){ ctx.fillStyle = p.type==='shield'?'#56ccf2':'#ffd166'; ctx.beginPath(); ctx.arc(p.x + p.size/2, p.y + p.size/2, p.size/2, 0, Math.PI*2); ctx.fill(); } const sel = car.selected || 1; const img = brandImgs[sel-1]; if(img && img.complete){ ctx.drawImage(img, car.x, car.y, car.w, car.h); } else { ctx.fillStyle='#ffd966'; ctx.fillRect(car.x,car.y,car.w,car.h); } ctx.setTransform(1,0,0,1,0,0); }

  function loop(){ update(); draw(); scoreEl.textContent = score; rafId=requestAnimationFrame(loop); }

  function startGame(){ // start the game loop only after name is set
    if(!player.name || player.name.trim().length===0) return;
    running = true; frame = 0; spawnTimer = 0; loop(); }
  ...
  startGameBtn.addEventListener('click', ()=>{
    ...
    if(!val){ alert('Bitte gib einen Namen ein.'); return; }
    player.name = val.substring(0,40);
  ...
  // init (do not auto-start the game loop) 
  ensurePlayer(); renderShop(); resizeEntities(); getHighscore(); renderTop10();

  // show name overlay if no name
  if(!player.name || player.name.trim().length===0){ nameOverlay.classList.remove('hidden'); } else { // if name exists, hide overlay and start
    nameOverlay.classList.add('hidden'); startGame(); }
